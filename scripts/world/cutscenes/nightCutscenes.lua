...
local nightCutsenes = {
    ...
    gate = function(cutscene)
        ...
        if cutscene:choicer({ "Climb over", "Brute force" }) == 1 then
            cutscene:text("* That's a pretty damn good idea![wait:5] Step on my head,[wait:5] Kris!", "smile")
            cutscene:detachFollowers()
            cutscene:detachCamera()
        ...
        cutscene:text("* So,[wait:5] we're going on another adventure?", "happy", "noelle")
        cutscene:setSpeaker(susie)
        cutscene:text("* Yeah![wait:5] Except,[wait:5] uh,[wait:5] this one might be a bit different from last time.")
        cutscene:setSpeaker(noelle)
        cutscene:text("* Really?[wait:5] Why would it be different?[wait:5] The last two times it's been about the same...", "happy", "noelle")
        cutscene:setSpeaker(susie)
        cutscene:text("* Well,[wait:5] we're actually checking out the old bunker.[wait:5] The doors are open now,[wait:5] so...")
        cutscene:setSpeaker(noelle)
        cutscene:text("* Oh,[wait:5] then why'd you think I'd wanna come to that creepy old place!?[wait:5] Fahaha!", "happy", "noelle")
        cutscene:setSpeaker(susie)
        cutscene:text("* Well,[wait:5] we were thinking that there's probably a source of the earthquakes in there...")
        cutscene:text("* And who knows,[wait:5] maybe we'll find some cool collectables in there or something!")
        cutscene:setSpeaker(noelle)
        cutscene:text("* When you put it like that,[wait:5] sure![wait:5] I'll come![wait:5] Let's just hurry out before my mom gets home...", "happy", "noelle")
    ...
    susieApartment = function (cutscene)
        ...
        cutscene:wait(1)
        cutscene:setSpeaker(noelle)
        cutscene:text("* Susie??[wait:5] Why was that name on the door not yours??")
        cutscene:setSpeaker(susie)
