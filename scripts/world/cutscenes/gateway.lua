...
function gateway.chasestart(cutscene)
    ...
    if Game["gateway_collapse_chase_started"] then
        ...
        local x, y = Game.world.map:getMarker("debris_impact")
        local debris = Sprite("world/cutscenes/gateway/debris_3", x, y)
        debris:setScale(2)
...
function gateway.fall_to_depths(cutscene)
    ...
    cutscene:wait(3)
    cutscene:fadeOut(8, {global = true})
    cutscene:wait(2)
    cutscene:wait(cutscene:panTo(Game.world.camera.x, Game.world.camera.y + 2000, 2, "in-cubic"))
