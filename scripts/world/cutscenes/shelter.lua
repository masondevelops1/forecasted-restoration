...
smasheddoor = function(cutscene, event)
    cutscene:text("* The door is smashed into itself.", "surprise_smile_b")
end;

lockeddoor1 = function(cutscene, event)
    cutscene:text("* (The door has a label on it.)", "confused_surprise")
    cutscene:text("* (\"MAIN STAIRWELL: PLEASE USE CAUTION!\")")
    cutscene:text("* (It's locked.)", "confused_surprise_b")
end;

lockeddoor2 = function(cutscene, event)
    cutscene:text("* (The door has a label on it.)", "confused_surprise")
    cutscene:text("* (\"THE GATEWAY AMIDST LIGHT & DARK: PLEASE USE CAUTION!\")")
    cutscene:text("* (It's locked.)", "confused_surprise_b")
end;

gatewayblocker = function(cutscene, event)
    cutscene:setSpeaker("susie")
    cutscene:text("* (I think we'd better check out other rooms before going here...)", "sus_nervous")
...
gatewayEnter = function(cutscene, event)
    if Plot:isBefore("gateway_enter") then
        ...
        cutscene:setSpeaker(susie)
        cutscene:text("* So...[wait:5] this is our only way into the Gateway...", "neutral_side")
        cutscene:text("* Man,[wait:5] it really feels like we're about to step into something big,[wait:5] huh?", "nervous")
        cutscene:text("* Earthquakes...[wait:5] creepy shelter...[wait:5] that stupid doctor...", "neutral")
        cutscene:text("* Can't believe it all leads up to this...", "neutral_side")
        cutscene:text("* Alright,[wait:5] we shouldn't waste anymore time.[wait:5]\n* C'mon,[wait:5] Kris!", "smile") 
...
firstTerminal = function(cutscene)
    if Plot:isBefore("gateway_terminal1") then
		...
		Kristal.Console:log("device_used")   
		cutscene:setSpeaker(susie)
		cutscene:text("* So...[wait:5] this is a terminal...?", "nervous")
        cutscene:text("* Well there's a big red button,[wait:5] so...[wait:5] go ahead and press that!", "smile") 
        cutscene:setSpeaker()
        Assets.playSound("item")
        cutscene:setSpeaker("voicemail")
        cutscene:text("[color:#ff00ff]> FLOOR B1 MAINTENANCE LOCK DEACTIVATED.\n[wait:5]> ELEVATOR IS NOW AVAILABLE.")
